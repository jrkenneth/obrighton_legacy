<?php
	...
	$message = "Enter your email or tenant ID to receive a password reset link.";
	$response_class = "text-muted";

	function ensureTenantPasswordResetTable($con) {
		$sql = "CREATE TABLE IF NOT EXISTS tenant_password_resets (
	...
	function buildResetLink($token, $tenant_id) {
		$scheme = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
		$host = $_SERVER['HTTP_HOST'] ?? 'localhost';
		$base_path = rtrim(dirname($_SERVER['PHP_SELF']), '/');
		return $scheme . '://' . $host . $base_path . '/set-password.php?tid=' . urlencode($tenant_id) . '&token=' . urlencode($token);
	}

	if (isset($_POST['request_password_reset'])) {
		CSRFProtection::checkToken($_POST['csrf_token'] ?? '');

		ensureTenantPasswordResetTable($con);

		$identifier = InputValidator::sanitizeText($_POST['identifier'] ?? '', 255);
		$lookup_stmt = $con->prepare("SELECT id, email, first_name, last_name FROM tenants WHERE email=? OR tenant_id=? LIMIT 1");
		...
		if ($lookup_stmt) {
			...
			if ($tenant) {
				...
				$delete_stmt = $con->prepare("DELETE FROM tenant_password_resets WHERE tenant_id=?");
				if ($delete_stmt) {
					$delete_stmt->bind_param("i", $tenant['id']);
					$delete_stmt->execute();
					$delete_stmt->close();
				}

				$insert_stmt = $con->prepare("INSERT INTO tenant_password_resets (tenant_id, token_hash, expires_at) VALUES (?, ?, ?)");
				if ($insert_stmt) {
					$insert_stmt->bind_param("iss", $tenant['id'], $token_hash, $expires_at);
					$insert_stmt->execute();
					$insert_stmt->close();
				}

				$reset_link = buildResetLink($token, $tenant['id']);
				...
				if (isHostedEnvironment()) {
					...
					if (function_exists('sendMail')) {
						...
						sendMail('no-reply@obrightonempire.com', 'O.Brighton Empire', $tenant['email'], $full_name, $subject, $body);
				...
				AuditLog::log('PASSWORD_RESET_REQUEST', 'tenants', (int)$tenant['id'], null, array('timestamp' => date('Y-m-d H:i:s')), 0);
			}
		}
...
<title>Tenant Portal - Forgot Password</title>
					...
					<div class="col-lg-6">
						<form method="POST">
							<div class="log-reg-form signup-modal form-style1 bgc-white p50 p30-sm default-box-shadow2 bdrs12">
								...
								<div class="mb25">
									<label class="form-label fw600 dark-color">Email / Tenant ID</label>
									<input type="text" name="identifier" class="form-control" placeholder="Enter your email or tenant ID" required>
								</div>
