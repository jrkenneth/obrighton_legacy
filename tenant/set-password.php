<?php
	...
	$tenant_id = InputValidator::validateInteger($tid, 1);

	$reset_row = null;

	if ($token_hash !== '' && $tenant_id) {
		$lookup_stmt = $con->prepare("SELECT id, tenant_id, expires_at FROM tenant_password_resets WHERE tenant_id=? AND token_hash=? LIMIT 1");
		if ($lookup_stmt) {
			$lookup_stmt->bind_param("is", $tenant_id, $token_hash);
			$lookup_stmt->execute();
	...
	if ($show_form && isset($_POST['set_new_password'])) {
		...
		if ($new_password === '' || $confirmed_password === '') {
			...
			$update_stmt = $con->prepare("UPDATE tenants SET password=?, password_status=2 WHERE id=?");
			if ($update_stmt) {
				$update_stmt->bind_param("si", $new_hash, $tenant_id);
				if ($update_stmt->execute()) {
					...
					AuditLog::log('PASSWORD_RESET', 'tenants', $tenant_id, null, array('timestamp' => date('Y-m-d H:i:s')), 0);
