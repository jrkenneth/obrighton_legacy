...
//--End Counts
    ...
    if(isset($_POST['login'])){
        ...
        if ($user === '' || $password === '') {
            ...
            $stmt = $con->prepare("SELECT id, password, first_name FROM tenants WHERE tenant_id=? LIMIT 1");
            $row = null;
            if ($stmt) {
    ...
    if(isset($_POST['create_new_ticket'])){	
        ...
        if ($title === '' || mb_strlen($title) > 255) {
            ...
            $_SESSION['response'] = 'error';
            $_SESSION['message'] = 'Please select a valid conversation type.';
            $_SESSION['expire'] = time() + 10;
        } else {
            $type_ok = false;
            ...
            if (!$type_ok) {
                $_SESSION['response'] = 'error';
                $_SESSION['message'] = 'Invalid conversation type selected.';
                $_SESSION['expire'] = time() + 10;
            } else {
    ...
    if(isset($_POST['submit_ticket_reply'])){	
        ...
        $complaint_id = '';
        $owner_stmt = $con->prepare("SELECT complaint_id FROM tickets WHERE id=? AND person_id=? AND target='tenants' LIMIT 1");
        if ($owner_stmt) {
            $person_id = (int)$this_tenant;
    ...
    if(isset($_GET['action'])){
        ...
        if($target == "delete-listing"){
            ...
            // Ownership check: listing must belong to tenant's current property
            $auth_stmt = $con->prepare("SELECT id FROM listings WHERE id=? AND property_id=? LIMIT 1");
            if (!$auth_stmt) {
                $_SESSION['response'] = 'error';
            ...
            $auth_stmt->bind_param('ii', $target_id, $tu_property_id);
            $auth_stmt->execute();
        ...
        if($target == "delete-media"){
            ...
            // Load + authorize media via listing property_id
            $tlm_listing_id = 0;
            $tlm_file_name = '';
            $load_stmt = $con->prepare("SELECT lm.listing_id, lm.file_name FROM listing_media lm JOIN listings l ON l.id=lm.listing_id WHERE lm.id=? AND l.property_id=? LIMIT 1");
            if ($load_stmt) {
                $load_stmt->bind_param('ii', $target_id, $tu_property_id);
                $load_stmt->execute();
