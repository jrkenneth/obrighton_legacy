<?php
  ...
  $stmt = $con->prepare("SELECT complaint_id, title, date_closed, status FROM tickets WHERE id=? AND person_id=? AND target='landlords' LIMIT 1");
  if ($stmt) {
    $stmt->bind_param('ii', $ticket_id, $this_landlord_id);
    $stmt->execute();
    $res = $stmt->get_result();
  ...
  $mark_stmt = $con->prepare("UPDATE ticket_messages SET status='1' WHERE complaint_id=? AND sender!='landlord'");
  if ($mark_stmt) {
